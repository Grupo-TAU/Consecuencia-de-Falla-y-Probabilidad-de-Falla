...
BUFFER_BUSQUEDA_DEFAULT     = 10.0   # metros — radio alrededor del punto medio del colector
...
def _clasificar_tipo(tipo_raw, mapping):
    ...
    # Subcadena — gana la clave mas larga
    best_len   = 0
    best_clase = 1
    for key, clase in mapping:
...
def _build_index_transformed(source, transformador=None):
    ...
    for feat in source.getFeatures():
        ...
        if transformador is not None:
            geom = feat.geometry()          # copia fresca para no mutar el original
...
class CfUbicacion(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        return (
            ...
            "El algoritmo calcula el PUNTO MEDIO de cada colector (al 50 % de su longitud) "
            "y crea un buffer de radio configurable (default 7 m) solo alrededor de ese punto. "
            "Esto evita capturar vias de esquinas o cruces ajenos al tramo principal.\n\n"
            "Se lee el campo TIPO de cada via que intersecta el buffer. Si hay varias vias "
            "de distinto tipo se asigna el valor mas alto (mayor consecuencia).\n\n"
            "Tabla por defecto:\n"
            "  1 = Sin pavimentar\n"
            "  2 = Local menor\n"
            "  3 = Local mayor\n"
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterString(
                PARAM_BUFFER,
                "Radio del buffer sobre el punto medio (metros)",
                defaultValue=str(BUFFER_BUSQUEDA_DEFAULT),
            )
        )
        self.addParameter(
            QgsProcessingParameterString(
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        try:
            buffer_dist = float(
                self.parameterAsString(parameters, PARAM_BUFFER, context)
                .strip().replace(",", ".")
            )
            if buffer_dist < 0:
                buffer_dist = BUFFER_BUSQUEDA_DEFAULT
        ...
        feedback.pushInfo(f"Campo salida clasificacion:      {campo_clasif}")
        feedback.pushInfo(f"Radio buffer sobre punto medio:  {buffer_dist} m")
        ...
        idx_tipo_via = _find_field_index(
            src_vias.fields(), (campo_tipo,), ("tipo",)
        )
        if idx_tipo_via == -1:
        ...
        feedback.pushInfo(
            f"Campo TIPO encontrado: '{src_vias.fields().at(idx_tipo_via).name()}'"
        )
        ...
        if crs_col.isValid() and crs_vias.isValid() and crs_col != crs_vias:
            transformador = QgsCoordinateTransform(
                crs_vias, crs_col, QgsProject.instance()
            )
            feedback.pushInfo(
                "CRS de Vias diferente al de Colectores — se transformara al vuelo."
            )

        # ── Construir indice espacial de Vias + mapa de atributos TIPO ────────
        ...
        attr_tipo = {}
        for feat in src_vias.getFeatures():
            attr_tipo[feat.id()] = feat[idx_tipo_via]

        feedback.pushInfo(f"Vias indexadas: {len(geom_vias)}")

        # ── Crear/verificar campo de salida en Colectores ─────────────────────
        ...
        try:
            for i, col in enumerate(colectores_list, start=1):
                ...
                if not col.hasGeometry():
                    feedback.pushInfo(f"  FID {col.id()}: sin geometria, se omite.")
                    continue
                ...
                # ── Punto medio al 50 % de la longitud del colector ───────────
                longitud = geom_col.length()
                if longitud <= 0:
                    feedback.pushInfo(
                        f"  FID {col.id()}: longitud 0, se omite."
                    )
                    sin_via += 1
                    continue
                ...
                if geom_punto_medio is None or geom_punto_medio.isEmpty():
                    feedback.pushInfo(
                        f"  FID {col.id()}: no se pudo calcular el punto medio, se omite."
                    )
                    sin_via += 1
                    continue

                # Buffer sobre el punto medio — evita capturar vias de esquinas
                geom_buf   = geom_punto_medio.buffer(buffer_dist, 8)
                candidatos = index_vias.intersects(geom_buf.boundingBox())

                clase_max = 0   # 0 = sin via encontrada todavia
                ...
                for fid_via in candidatos:
                    ...
                    tipo_raw = attr_tipo.get(fid_via)
                    clase    = _clasificar_tipo(tipo_raw, tipo_mapping)
                    if clase > clase_max:
                        clase_max = clase

                # Sin via → clase 1 (Sin pavimentar)
                nueva_clase = clase_max if clase_max > 0 else 1
                if clase_max == 0:
                    sin_via += 1

                valor_actual = col[idx_clasif]
                if valor_actual != nueva_clase:
                    if not capa_colectores.changeAttributeValue(
                        col.id(), idx_clasif, nueva_clase
                    ):
                        raise QgsProcessingException(
        ...
        if sin_via:
            feedback.pushInfo(
                f"Advertencia: {sin_via} colectores sin via encontrada en el punto medio "
                f"(radio {buffer_dist} m) → asignados clase 1."
            )
