...
@alg(name="actualizar_colectores_long_zamp_pend",
     label="Actualizar Colectores Longitud, Cota Zampeado y Pendiente",
...
def actualizar_colectores_long_zamp_pend(instance, parameters, context, feedback, inputs):
    ...
    if idx_longitud == -1:
        ...
        idx_longitud = _find_field_index(colectores_fields, ("Longitud", "LONGITUD", "longitud"))
        if idx_longitud == -1:
            raise QgsProcessingException("El campo Longitud no quedo disponible en Colectores.")
    ...
    if not colectores_layer.isEditable():
        if not colectores_layer.startEditing():
            raise QgsProcessingException("No se pudo iniciar el modo de edición en Colectores.")
        inicio_edicion = True
    ...
    try:
        for i, feature in enumerate(colectores_list, start=1):
            ...
            if (
                ...
                pendiente_calc = round(((cota_ini_val - cota_fin_val) / long_aux_val) * 100.0, 2)
